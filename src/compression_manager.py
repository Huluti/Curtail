...
class CompressionManager:
    ...
    def compress(self, result_items, update_result_item, enable_compression):
        threading.Thread(
            target=self._compress,
            args=(result_items, update_result_item, enable_compression),
            daemon=True,
        ).start()

    def _compress(self, result_items, update_result_item, enable_compression):
        ...
        for result_item in result_items:
            ...
            future = executor.submit(self.compressors[file_type].run, result_item)
            futures.append(future)

        for future in futures:
            future.result()
        GLib.idle_add(enable_compression, True)
